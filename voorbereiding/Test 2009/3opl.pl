...
#   stap 3: komt een attribuut maximaal éénmaal voor in de verplichte attributen van een klasse ? 
...
foreach $klasse (@ldap){
    ...
    if($klasse =~ /[.]*objectClassCategory:[\s]*(.*)/){
      $type = $1;
      $ldap{$naam}->{type} = $type;
    }

    if($klasse =~ /[.]*subClassOf:[\s]*(.*)/){
      $ouderKlasse = $1;
      $ldap{$naam}->{ouderKlasse} = $ouderKlasse;
    }

    while($klasse =~ /[.]*systemMustContain:[\s]*(.*)/g){
      push(@{$ldap{$naam}->{verplAttr}}, $1);
    }

    while($klasse =~ /[.]*mustContain:[\s]*(.*)/g){
      push(@{$ldap{$naam}->{verplAttr}}, $1);
    }

    while($klasse =~ /[.]*auxiliaryClass:[\s]*(.*)/g){
      push(@{$ldap{$naam}->{hulpKlasse}}, $1);
    }

    while($klasse =~ /[.]*systemAuxiliaryClass:[\s]*(.*)/g){
      push(@{$ldap{$naam}->{hulpKlasse}}, $1);
    }
}

#Verplichte attributen aanvullen voor stap2:
foreach $key (keys %ldap){
  foreach $hulpklasse (@{$ldap{$key}->{hulpKlasse}}){
    push (@{$ldap{$key}->{verplAttr}},@{$ldap{$hulpklasse}->{verplAttr}});
  }
}

#Verplichte attributen aanvullen voor stap3:
foreach $key (keys %ldap){
  $refouder = $ldap{$key}->{ouderKlasse};
  while($refouder){
    foreach $ouderAttribuut (@{$ldap{$refouder}->{verplAttr}}){
      $uniq = 1;
      foreach $attribuut (@{$ldap{$key}->{verplAttr}}){
	if($attribuut eq $ouderAttribuut){
	  $uniq = 0;
	}
      }
      if($uniq){
	push(@{$ldap{$key}->{verplAttr}},$ouderAttribuut);
      }
    }
    $refouder = $ldap{$refouder}->{ouderKlasse};
  }
}

#Uitvoer voor stap 4
$i=1;
foreach $key (sort keys %ldap){
  $alone = 1;
  foreach $ouderKey (sort keys %ldap){
    if($ldap{$key}->{naam} eq $ldap{$ouderKey}->{ouderKlasse}){
      $alone = 0;
      last;
    }
  }
  if($ldap{$key}->{type} == 1 &&
     $ldap{$key}->{ouderKlasse} ne "top" &&
     $alone){
    print "$i: $ldap{$key}->{naam}\n";
    print "Ouder: $ldap{$key}->{ouderKlasse}\n";
    print "Type: $ldap{$key}->{type}\n\n";
    $i++;
  }
}

#Uitvoer voor stap 1, 2 en 3
#$i=1;
#foreach $key (sort keys %ldap){
#  if($#{$ldap{$key}->{verplAttr}} != -1){
#    print "$i: $ldap{$key}->{naam}:";
#    foreach $verplAttr (sort @{$ldap{$key}->{verplAttr}}){
#       print "$verplAttr|";
#    }
#    print "\n";
#    $i++;
